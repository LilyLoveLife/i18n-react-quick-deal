...
var isInConsole = function (path) {
    return path.findParent(function (p) { return p.isCallExpression(); }) && path.parent.callee.object.name === 'console';
};
var flag_garther = false;
var toTranslateSet = new Set();
var addHook = function (programNode) {
    programNode.body;
};
var getNewContent = function (filePath) {
    ...
    traverse(ast, {
        Program: function (_a) {
            var node = _a.node, parantPath = _a.parantPath;
        ...
        ArrowFunctionExpression: function (path) {
            var parentFunctionPath = path.findParent(function (p) { return p.isArrowFunctionExpression() || p.isFunctionExpression(); });
...
var dealFile = function (filePath) {
    ...
    if (fileTypeList.includes(path.extname(filePath))) {
        ...
        // 写入新内容到文件
        fs.writeFile(newFilePath, newContent.code, 'utf8', function (writeErr) {
            if (writeErr) {
