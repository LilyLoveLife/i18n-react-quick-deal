...
const getNewContent = (filePath: string) => {
  ...
  traverse(ast, {
    Program({
      ...
      const importList = (node.body.filter((item: {
        type: string;
      }) => item.type === 'ImportDeclaration') as ImportDeclaration[]);
    ...
    ArrowFunctionExpression(path) {
      ...
      if (isTopFunctionPath) {
        ...
        console.log(t('找到最外层的函数:'), path.node); // , path.node.argument
